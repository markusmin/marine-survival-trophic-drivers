...
using namespace density;
  ...
  // DATA_MATRIX(Sigma_pianka_o_cssif_prey_field_t);  // covariance matrix for index of abundance for cssif
  // because we can't ADReport() this index to get the covariance, here we will just treat this as a fixed measurement error.
  
  DATA_MATRIX(Sigma_pianka_o_cssif_prey_field_t); // estimated standard error for overlap metric with prey field
  DATA_MATRIX(Sigma_pianka_o_cssif_rf_t); // estimated standard error for overlap metric with rf
  DATA_MATRIX(Sigma_pianka_o_cssif_sosh_t); // estimated standard error for overlap metric with sosh
  DATA_MATRIX(Sigma_pianka_o_cssif_comu_t); // estimated standard error for overlap metric with comu
  ...
  PARAMETER_VECTOR(pianka_o_cssif_prey_field_t_latent);
  PARAMETER_VECTOR(rf_index_t_latent);
  PARAMETER_VECTOR(pianka_o_cssif_rf_t_latent);
  PARAMETER_VECTOR(sosh_index_t_latent);
  PARAMETER_VECTOR(pianka_o_cssif_sosh_t_latent);
  PARAMETER_VECTOR(comu_index_t_latent);
  PARAMETER_VECTOR(pianka_o_cssif_comu_t_latent);
  ...
  // cssif_prey_field overlap metric
  vector<Type> residual_pianka_o_cssif_prey_field_t(n_t);
  MVNORM_t<Type> neg_log_dmvnorm_pianka_o_cssif_prey_field(Sigma_pianka_o_cssif_prey_field_t);
  residual_pianka_o_cssif_prey_field_t = vector<Type>(pianka_o_cssif_prey_field_t - pianka_o_cssif_prey_field_t_latent); // here we are calculating the residuals between the data and the underlying latent state
  jnll += neg_log_dmvnorm_pianka_o_cssif_prey_field(residual_pianka_o_cssif_prey_field_t); // then we use those residuals and evaluate them
  
  // cssif_rf overlap metric
  vector<Type> residual_pianka_o_cssif_rf_t(n_t);
  MVNORM_t<Type> neg_log_dmvnorm_pianka_o_cssif_rf(Sigma_pianka_o_cssif_rf_t);
  residual_pianka_o_cssif_rf_t = vector<Type>(pianka_o_cssif_rf_t - pianka_o_cssif_rf_t_latent); // here we are calculating the residuals between the data and the underlying latent state
  jnll += neg_log_dmvnorm_pianka_o_cssif_rf(residual_pianka_o_cssif_rf_t); // then we use those residuals and evaluate them
  
  // cssif_sosh overlap metric
  vector<Type> residual_pianka_o_cssif_sosh_t(n_t);
  MVNORM_t<Type> neg_log_dmvnorm_pianka_o_cssif_sosh(Sigma_pianka_o_cssif_sosh_t);
  residual_pianka_o_cssif_sosh_t = vector<Type>(pianka_o_cssif_sosh_t - pianka_o_cssif_sosh_t_latent); // here we are calculating the residuals between the data and the underlying latent state
  jnll += neg_log_dmvnorm_pianka_o_cssif_sosh(residual_pianka_o_cssif_sosh_t); // then we use those residuals and evaluate them
  
  // cssif_comu overlap metric
  vector<Type> residual_pianka_o_cssif_comu_t(n_t);
  MVNORM_t<Type> neg_log_dmvnorm_pianka_o_cssif_comu(Sigma_pianka_o_cssif_comu_t);
  residual_pianka_o_cssif_comu_t = vector<Type>(pianka_o_cssif_comu_t - pianka_o_cssif_comu_t_latent); // here we are calculating the residuals between the data and the underlying latent state
  jnll += neg_log_dmvnorm_pianka_o_cssif_comu(residual_pianka_o_cssif_comu_t); // then we use those residuals and evaluate the
  ...
  for( int i=0; i<z_i.size(); i++){
    ...
    beta_UCSF * UCSF_i(i) +
      ...
      beta_ov_prey_field*pianka_o_cssif_prey_field_t_latent(t_i(i)) +
      beta_rf_index*rf_index_t_latent(t_i(i)) +
      beta_ov_rf*pianka_o_cssif_rf_t_latent(t_i(i)) +
      beta_sosh_index*sosh_index_t_latent(t_i(i)) +
      beta_ov_sosh*pianka_o_cssif_sosh_t_latent(t_i(i)) +
      beta_comu_index*comu_index_t_latent(t_i(i)) +
      beta_ov_comu*pianka_o_cssif_comu_t_latent(t_i(i)),
  ...
  REPORT(pianka_o_cssif_prey_field_t_latent);
  REPORT(rf_index_t_latent);
  REPORT(pianka_o_cssif_rf_t_latent);
  REPORT(sosh_index_t_latent);
  REPORT(pianka_o_cssif_sosh_t_latent);
  REPORT(comu_index_t_latent);
  REPORT(pianka_o_cssif_comu_t_latent);
