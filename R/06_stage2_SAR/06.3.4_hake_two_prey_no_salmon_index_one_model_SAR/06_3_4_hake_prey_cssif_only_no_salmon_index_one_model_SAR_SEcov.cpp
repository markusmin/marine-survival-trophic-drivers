...
using namespace density;
  ...
  DATA_MATRIX(Sigma_rf_index_t);  // covariance matrix for index of abundance for prey field
  DATA_MATRIX(Sigma_hake_index_t);  // covariance matrix for index of abundance for prey field
  ...
  DATA_MATRIX(Sigma_pianka_o_cssif_rf_t); // estimated standard error for overlap metric with prey field
  DATA_MATRIX(Sigma_pianka_o_cssif_hake_t); // estimated standard error for overlap metric with hake
  ...
  PARAMETER_VECTOR(hake_index_t_latent);
  PARAMETER_VECTOR(pianka_o_cssif_hake_t_latent);
  ...
  // hake index of abundance
  vector<Type> residual_hake_index_t(n_t);
  MVNORM_t<Type> neg_log_dmvnorm_hake_index(Sigma_hake_index_t);
  residual_hake_index_t = vector<Type>(hake_index_t - hake_index_t_latent); // here we are calculating the residuals between the data and the underlying latent state
  jnll += neg_log_dmvnorm_hake_index(residual_hake_index_t); // then we use those residuals and evaluate them
  ...
  // cssif_hake overlap metric
  vector<Type> residual_pianka_o_cssif_hake_t(n_t);
  MVNORM_t<Type> neg_log_dmvnorm_pianka_o_cssif_hake(Sigma_pianka_o_cssif_hake_t);
  residual_pianka_o_cssif_hake_t = vector<Type>(pianka_o_cssif_hake_t - pianka_o_cssif_hake_t_latent); // here we are calculating the residuals between the data and the underlying latent state
  jnll += neg_log_dmvnorm_pianka_o_cssif_hake(residual_pianka_o_cssif_hake_t); // then we use those residuals and evaluate them
  ...
  for( int i=0; i<z_i.size(); i++){
    ...
    jnll -= dbinom_robust(z_i(i), Type(1),
      ...
      beta_hake_index*hake_index_t_latent(t_i(i)) +
      beta_ov_hake*pianka_o_cssif_hake_t_latent(t_i(i)),
  ...
  REPORT(hake_index_t_latent);
  REPORT(pianka_o_cssif_hake_t_latent);
