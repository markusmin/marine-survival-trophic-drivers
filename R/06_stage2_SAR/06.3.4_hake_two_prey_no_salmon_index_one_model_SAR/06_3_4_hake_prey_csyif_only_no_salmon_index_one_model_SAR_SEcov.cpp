...
using namespace density;
  ...
  DATA_VECTOR(pianka_o_csyif_hake_t);
  ...
  DATA_MATRIX(Sigma_pianka_o_csyif_rf_t); // estimated standard error for overlap metric with prey field
  DATA_MATRIX(Sigma_pianka_o_csyif_hake_t); // estimated standard error for overlap metric with hake
  ...
  PARAMETER_VECTOR(pianka_o_csyif_hake_t_latent);
  ...
  // csyif_hake overlap metric
  vector<Type> residual_pianka_o_csyif_hake_t(n_t);
  MVNORM_t<Type> neg_log_dmvnorm_pianka_o_csyif_hake(Sigma_pianka_o_csyif_hake_t);
  residual_pianka_o_csyif_hake_t = vector<Type>(pianka_o_csyif_hake_t - pianka_o_csyif_hake_t_latent); // here we are calculating the residuals between the data and the underlying latent state
  jnll += neg_log_dmvnorm_pianka_o_csyif_hake(residual_pianka_o_csyif_hake_t); // then we use those residuals and evaluate them
  ...
  for( int i=0; i<z_i.size(); i++){
    ...
    beta_UCSF * UCSF_i(i) +
      ...
      beta_ov_hake*pianka_o_csyif_hake_t_latent(t_i(i)),
  ...
  REPORT(pianka_o_csyif_hake_t_latent);
