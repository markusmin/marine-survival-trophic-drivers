...
using namespace density;
  ...
  for( int i=0; i<z_i.size(); i++){
    ...
    jnll -= dbinom_robust(z_i(i), Type(1),
      ...
      beta_ov_prey_field*pianka_o_csyif_prey_field_t(t_i(i)) +
      beta_rf_index*rf_index_t(t_i(i)) +
      beta_ov_rf*pianka_o_csyif_rf_t(t_i(i)) +
      beta_sosh_index*sosh_index_t(t_i(i)) +
      beta_ov_sosh*pianka_o_csyif_sosh_t(t_i(i)) +
      beta_comu_index*comu_index_t(t_i(i)) +
      beta_ov_comu*pianka_o_csyif_comu_t(t_i(i)),
