// Model 3-5 - minus distance from shore
...
//using namespace R_inla;  // Not loaded globally, but used below for anisotropy
  ...
  for( int i=0; i<D_i.size(); i++){
    dhat_i(i) = exp( beta_t(t_i(i)) + epsilon_it(i, t_i(i)) + omega_i(i) +
      + beta_dist*dist_i(i)); //beta_temp*temp_i(i)); 
  ...
  for( int t=0; t<n_t; t++){
    for( int g=0; g<n_g; g++){
      ln_d_gt(g,t) = beta_t(t) + beta_dist*dist_g(g) + omega_g(g) + epsilon_gt(g,t) ; // + beta_temp*temp_gt(g,t)
    }
  }
