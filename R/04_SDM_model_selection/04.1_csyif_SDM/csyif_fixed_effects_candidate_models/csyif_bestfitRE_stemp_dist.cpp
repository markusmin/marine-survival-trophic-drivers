// Best fit RE model, with density ~ dist + s(SST)
...
//using namespace R_inla;  // Not loaded globally, but used below for anisotropy
  ...
  for( int i=0; i<D_i.size(); i++){
    dhat_i(i) = exp( epsilon_it(i, t_i(i)) + omega_i(i) + beta_dist*dist_i(i) +
  ...
  for( int t=0; t<n_t; t++){
    for( int g=0; g<n_g; g++){
      ...
      ln_d_gt(g,t) = omega_g(g) + epsilon_gt(g,t) + beta_dist*dist_g(g) + proj_smooth_i(g + n_g * t) ;
    }
  }
