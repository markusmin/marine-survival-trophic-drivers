// Best fit RE model, with density ~ factor(year) + s(dist) + SST
...
//using namespace R_inla;  // Not loaded globally, but used below for anisotropy
  ...
  for( int i=0; i<D_i.size(); i++){
    dhat_i(i) = exp( beta_t(year_factor_t_i(t_i(i))) + epsilon_it(i, t_i(i))  + beta_temp*temp_i(i) +
  ...
  for( int t=0; t<n_t; t++){
    if( year_factor_t_i(t)==-999 ){
      for( int g=0; g<n_g; g++){
        ln_d_gt(g,t) = beta_temp*temp_gt(g,t) + epsilon_gt(g,t) + proj_smooth_i(g + n_g * t) ;
      }  
    }else{
      for( int g=0; g<n_g; g++){
        ln_d_gt(g,t) = beta_t(year_factor_t_i(t)) + beta_temp*temp_gt(g,t) + epsilon_gt(g,t) + proj_smooth_i(g + n_g * t) ;
      }
    }
  }
