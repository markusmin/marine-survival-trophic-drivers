...
//using namespace R_inla;  // Not loaded globally, but used below for anisotropy
  ...
  for( int t=0; t<n_t; t++){
    if( year_factor_t_i(t)==-999 ){
      for( int g=0; g<n_g; g++){
        ln_d_gt(g,t) = beta_temp*temp_gt(g,t) + epsilon_gt(g,t) ; // + beta_dist*dist_g(g) 
      }  
    }else{
      for( int g=0; g<n_g; g++){
        ln_d_gt(g,t) = beta_t(year_factor_t_i(t)) + beta_temp*temp_gt(g,t) + epsilon_gt(g,t) ; // + beta_dist*dist_g(g) 
      }
    }
  }
