...
// #### HAKE ####
  ...
  // create a vector to store the index of abundance and calculate it for each year
  vector<Type> hake_index_of_abundance(n_t_jsoes);
  for( int t=0; t<n_t_jsoes; t++){
...
array<Type> d_gt_hake_sq( n_g_csyif, n_t_shared); // array to store the squared proportional densities of hake in each cell (for denominators)
// calculate the squared densities for the denominator
...
for( int t=0; t<n_t_shared; t++){
  for( int g=0; g<n_g_csyif; g++){
    ov_gt_num_csyif_hake(g,t) = d_gt_csyif(g,t_jsoes_indices(t))/d_gt_csyif.col(t_jsoes_indices(t)).sum() * d_gt_hake(g,t)/d_gt_hake.col(t).sum(); // calculate value for each cell
  }
}
...
for( int t=0; t<n_t_shared; t++){
  pianka_o_csyif_hake_t(t) = ov_gt_num_csyif_hake.col(t).sum()/sqrt(d_gt_csyif_sq.col(t_jsoes_indices(t)).sum()*d_gt_hake_sq.col(t).sum());
}
...
for( int t=0; t<n_t_shared; t++){
  for( int g=0; g<n_g_cssif; g++){
    ov_gt_num_cssif_hake(g,t) = d_gt_cssif(g,t_jsoes_indices(t))/d_gt_cssif.col(t_jsoes_indices(t)).sum() * d_gt_hake(g,t)/d_gt_hake.col(t).sum(); // calculate value for each cell
  }
}
...
// calculate the index for each year
for( int t=0; t<n_t_shared; t++){
  pianka_o_cssif_hake_t(t) = ov_gt_num_cssif_hake.col(t).sum()/sqrt(d_gt_cssif_sq.col(t_jsoes_indices(t)).sum()*d_gt_hake_sq.col(t).sum());
}
