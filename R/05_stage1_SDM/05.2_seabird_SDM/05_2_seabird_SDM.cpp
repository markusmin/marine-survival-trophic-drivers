...
DATA_INTEGER(n_t_shared_comu); // number of years in the dataset
...
// vector<Type> omega_g( n_g );
// omega_g = A_gs * omega_s;
matrix<Type> epsilon_gt_sosh( n_g_sosh, n_t_shared );
...
// vector<Type> omega_i( n_i );
// omega_i = A_is * omega_s;
matrix<Type> epsilon_it_sosh( n_i_sosh, n_t_shared );
...
// Note that the projection grid is the same for all species - so temp_gt and dist_g are universal, and the number of years (t) are the same
array<Type> ln_d_gt_sosh( n_g_sosh, n_t_shared );

// estimate the smooth effects in the projection grid
// smoothers for projection grid
vector<Type> proj_smooth_i_sosh(n_g_sosh*n_t_shared);
...
// sum across all cells, multiply by the area of each cell
// convert to normal space
array<Type> d_gt_sosh( n_g_sosh, n_t_shared );
...
// create a vector to store the index of abundance and calculate it for each year
vector<Type> sosh_index_of_abundance(n_t_shared);
for( int t=0; t<n_t_shared; t++){
...
array<Type> d_gt_sosh_sq( n_g_csyif, n_t_shared ); // array to store the squared proportional densities of sosh in each cell (for denominators)
// calculate the squared densities for the denominator
for( int t=0; t<n_t_shared; t++){
  for( int g=0; g<n_g_csyif; g++){
...
for( int t=0; t<n_t_shared; t++){
  for( int g=0; g<n_g_csyif; g++){
    ov_gt_num_csyif_sosh(g,t) = d_gt_csyif(g,t_jsoes_indices(t))/d_gt_csyif.col(t_jsoes_indices(t)).sum() * d_gt_sosh(g,t)/d_gt_sosh.col(t).sum(); // calculate value for each cell
  }
}
...
// calculate the index for each year
for( int t=0; t<n_t_shared; t++){
  pianka_o_csyif_sosh_t(t) = ov_gt_num_csyif_sosh.col(t).sum()/sqrt(d_gt_csyif_sq.col(t_jsoes_indices(t)).sum()*d_gt_sosh_sq.col(t).sum());
}
...
for( int t=0; t<n_t_shared; t++){
  for( int g=0; g<n_g_cssif; g++){
    ov_gt_num_cssif_sosh(g,t) = d_gt_cssif(g,t_jsoes_indices(t))/d_gt_cssif.col(t_jsoes_indices(t)).sum() * d_gt_sosh(g,t)/d_gt_sosh.col(t).sum(); // calculate value for each cell
  }
}
...
// calculate the index for each year
for( int t=0; t<n_t_shared; t++){
  pianka_o_cssif_sosh_t(t) = ov_gt_num_cssif_sosh.col(t).sum()/sqrt(d_gt_cssif_sq.col(t_jsoes_indices(t)).sum()*d_gt_sosh_sq.col(t).sum());
}
